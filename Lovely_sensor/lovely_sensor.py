...
--------------------------------------------------------------------
使い方:
    同じディレクトリに config_lovely.yaml を置いて
        python lovely_sensor.py
    だけで実行できる。設定は config_lovely.yaml 側で変更する。
--------------------------------------------------------------------
"""
...
def process_one(in_path, cfg):
    ...
    shock_mask = mask.astype(np.int32)
    ...
    print(f"  衝撃波判定節点数   : {shock_mask.sum()}  (M_n>={mn_threshold}"
...
def main():
    ...
    print("完了 (ParaView で normal_mach / shock_mask を可視化)")
